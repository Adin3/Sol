...
void Renderer::CreateQuad(const glm::vec2& size, const glm::vec2& position, const unsigned int& textureID, int frames, int& frame, float speed, float& time, float deltatime, const glm::vec4& color)
{
	if (s_data.indicesCount >= maxIndices || s_data.textureCount >= 31)
	{
		EndBatch();
	...
	s_data.rendererBufferptr->Position = secx * pos[0];
	s_data.rendererBufferptr->Color = color;
	s_data.rendererBufferptr->TexCoords = { wTex * frame, 0.0f };
	s_data.rendererBufferptr->TextureID = index;
	s_data.rendererBufferptr++;

	s_data.rendererBufferptr->Position = secx * pos[1];
	s_data.rendererBufferptr->Color = color;
	s_data.rendererBufferptr->TexCoords = { wTex * frame, 1.0f };
	s_data.rendererBufferptr->TextureID = index;
	s_data.rendererBufferptr++;

	s_data.rendererBufferptr->Position = secx * pos[2];
	s_data.rendererBufferptr->Color = color;
	s_data.rendererBufferptr->TexCoords = { wTex * (frame+1), 1.0f };
	s_data.rendererBufferptr->TextureID = index;
	s_data.rendererBufferptr++;

	s_data.rendererBufferptr->Position = secx * pos[3];
	s_data.rendererBufferptr->Color = color;
	s_data.rendererBufferptr->TexCoords = { wTex * (frame + 1), 0.0f };
	s_data.rendererBufferptr->TextureID = index;
	s_data.rendererBufferptr++;
...
Renderer::Renderer()
{
	...
	shader = std::make_shared<Shader>("shader/rec/vshader.vs", "shader/rec/fshader.fs");
